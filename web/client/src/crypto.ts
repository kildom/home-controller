 ...
 * @returns 
 */
export async function deriveKeys(passwordStr: string, saltHex: string, xHex: string, yHex: string) {
...
export async function sign(messageHex, prv) {
...
export async function verify(messageHex, sigHex, xHex, yHex) {
    let messageBytes = hexToBytes(messageHex);
...
export async function createAuthFile(password: string): Promise<string> {
    ...
    let authData = {
        ...
        salt: keys.salt,
        ch1: getRandomHex(32),
        // TODO: Encrypt private key with AES using password-derived key
    };
    return JSON.stringify(authData, null, 4);
}
