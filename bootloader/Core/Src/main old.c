...
static void switchHandler(NetworkPort *port, bool headerReceived)
{
  port->state.headerReceived = headerReceived;
  port->state.index = 0;
  port->state.rxValueOffset = 0;
}

static void receiveHeader(NetworkPort *port)
{
  int byte;

  byte = rxReceive(port);
  if (byte != magicBytes[port->state.index]) {
    port->state.index = 0;
  } else {
    port->state.index++;
    if (port->state.index == magicBytesLength) {
      switchHandler(port, true);
    }
  }
}

static void receiveContent(NetworkPort *port)
{
  int byte;
  ...
  if (byte == NO_DATA) {
    ...
    port->state.rxValueOffset = byte;

  } else {

    byte += port->state.rxValueOffset;
    port->state.rxValueOffset = 0;

    if (byte == 0x100) {

      if (sendPacketLength > 0) {
        for (int i = 0; i < sendPacketLength; i++) {
          txSend(port, sendPacket[i]); // Contains also ESC+END sequence
        }
        sendPacketLength = 0;
      }
    ...
    } else if (port->state.index < sizeof(port->buffer)) {

      port->buffer[port->state.index++] = byte;
...
void receiveHandler(NetworkPort *port)
{
  if (port->state.headerReceived) {
    receiveContent(port);
  } else {
    receiveHeader(port);
  }
...
MAGIC BYTES (5 bytes):
ESC   0xAA   - start of packet
mask  0x00   - boot packet will never contain 0xAA
flags 0x21   - bootloader protocol, one destination node
dst   0xFF   - special address reserved for bootloader
type  0xB1   - bootloader packet type

Identification (14 bytes):
uid_len      - length of UID
uid          - device UID

Operation (6 bytes):
type         - operation type: query, reset, erase, write, read
address      - address in flash memory
len          - length of the buffer for read/write operations

Buffer (64 bytes, only for write operations):
data         - data

crc32
end

end_mark 0xFF - end of data
dummy 0x
